    ...
    <div class="layui-fluid">
        <div class="layui-row">
            <form class="layui-form">
                ...
                @if(!$manager->root)
                <div class="layui-form-item">
                    <label for="platform_id" class="layui-form-label">
                        <span class="x-red">*</span>所属平台</label>
                    <div class="layui-input-inline">
                        <select id="platform_id" name="platform_id" class="valid">
                            @foreach($GetPfNameId as $v)
                            <option value="{{$v->platform_id}}" @if($v->platform_status == 2) disabled @endif @if($v->platform_id == $manager->platform_id) selected @endif>{{$v->pf_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
