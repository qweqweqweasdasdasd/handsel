...
class BaiJiaPoint
{
	 ...
	 *	图片路径
	 */
	protected $codePath = '/Captche/Baijia';

	/**
	 *	cookies 保存键名
	 */
	protected $cookieKey = 'bj-cookie';
	...
	public function VerifyPlatformUser($username = '')
	{
		...
		$opt = [
			'timeout' => 30,
			'headers' => [
				'Authorization' => Di::getInstance()->get('REDIS')->get($this->cookieKey),
			]
		];
	 ...
	 *	心跳包
	 *	get
	 *	http://bjqpadmin1.com:1788/adminsystem/server/homepage/findByWithdrawCount
	 */
	...
	public function GetAuthorization()
	{
		...
		if($res['status'] == 1){
			...
			Di::getInstance()->get('REDIS')->set($this->cookieKey,$data['Authorization']);
	...
	public function ParseCode()
	{
		...
		// 把base64 转换为普通图片
		if(!is_dir(EASYSWOOLE_ROOT.$this->codePath)){
			mkdir(EASYSWOOLE_ROOT.$this->codePath,0777,true);
		}
		file_put_contents(EASYSWOOLE_ROOT.$this->codePath.'/code.png',base64_decode($base64));

		try {
          	//echo public_path().'/code.png';
    		$num = $this->juheApi(EASYSWOOLE_ROOT.$this->codePath.'/code.png');
          	//dd($num);
