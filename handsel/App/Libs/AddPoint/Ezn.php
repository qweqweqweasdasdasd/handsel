...
class Ezn
{
	...
	public function AutoHandsel($platform,$username,$phone,$money,$other=[])
	{
        ...
        //var_dump('ok');
		$fp = new \App\Libs\Platform\EznPoint($platform);
		//var_dump($fp);
        try {
            ...
            switch ($res['code']) {
                case '1':
                	$data['user_id'] = json_decode($res['data'])->user_id;
                	var_dump($data);
	...
	public function DoPlatformPointTest($data = [])
    {
        ...
        $bj = new \App\Libs\Platform\EznPoint($platform);
	...
	{
		$fp = new \App\Libs\Platform\EznPoint();
